...
static inline
REAL8
CalculateCrossProduct( const INT i, const REAL8 a[], const REAL8 b[] )
{
...
//print_debug( "Current funcvals = %.16e %.16e %.16e\n", gsl_vector_get( f, 0 ), gsl_vector_get( f, 1 ),
...
//print_debug("qCart[0] = %.3f, pCart = %.3f, %.3f\n", qCart[0], pCart[1], pCart[2]);
...
//print_debug( "d2Hdr2 = %.16e d2Hdrdpphi = %.16e dHdpphi = %.16e\n", d2Hdr2, d2Hdrdpphi, dHdpphi );
  ...
  if ( d2Hdr2 != 0.0 )
  {
    ...
    CalculateSigmaKerr( &sKerr, mass1, mass2, &s1Vec, &s2Vec );
    CalculateSigmaStar( &sStar, mass1, mass2, &s1Vec, &s2Vec );
    ...
    ham = SpinEOBHamiltonian( eta, &qCartVec, &pCartVec, &s1VecNorm, &s2VecNorm, &sKerr, &sStar, params->tortoise, params->seobCoeffs );
  ...
  //print_debug( "flux = %.16e, dEdr = %.16e, dHdpr = %.16e\n", flux, dEdr, dHdpr );
...
//print_debug( "Applying the tortoise to p (csi = %.26e)\n", csi );
...
//print_debug( "THE FINAL INITIAL CONDITIONS:\n");
