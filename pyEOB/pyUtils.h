...
#define MACRO_REAL8Vector2PyArray(vec, ret) _REAL8Vector2PyArray(vec, ret, __UNIQUE_ID(_id))
#define _REAL8Vector2PyArray(vec, ret, x) \
npy_intp __PASTE(x, length)[]={0}; \
__PASTE(x, length)[0]=vec->length; \
REAL8 *__PASTE(x, retmem); \
__PASTE(x, retmem) = (REAL8 *)malloc(vec->length*sizeof(*__PASTE(x, retmem))); \
memcpy(__PASTE(x, retmem), vec->data, vec->length*sizeof(REAL8)); \
PyObject *ret = PyArray_SimpleNewFromData(1, __PASTE(x, length), NPY_DOUBLE, (void*)__PASTE(x, retmem)); \
